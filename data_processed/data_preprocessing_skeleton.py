...
output_filepath = os.path.join(output_dir, 'data_processed_allStreams_60hz_onlyForehand_skeleton_skill_level.hdf5') # output file name
...
device_streams_for_features = [
    ('pns-joint', 'global-position', identity),
]

# Load/filter uses the full-modality requirement to keep sample counts consistent.
device_streams_required = [
    ('pns-joint', 'global-position', identity),
...
device_streams_unique = list(dict.fromkeys((device, stream) for device, stream, _ in device_streams_required))
device_streams_by_device = defaultdict(set)
for device, stream, _ in device_streams_required:
    device_streams_by_device[device].add(stream)

streams_for_alignment = device_streams_required
output_stream_key = ('pns-joint', 'global-position')
output_stream_index = next(
    idx for idx, (dev, stream, _) in enumerate(streams_for_alignment)
    if (dev, stream) == output_stream_key
)
...
for (subject_id, file_datas) in data_bySubject.items():
    ...
    for (data_file_index, file_data) in enumerate(file_datas):
        ...
        feature_segments_per_stream = [None] * len(streams_for_alignment)
        for device_idx, (device_name, stream_name, extractor) in enumerate(streams_for_alignment):
            ...
            for j in range(len(Forehand_start_time_list)):
                ...
                if len(high_time_indexes[0]) > 0:
                    ...
                    if device_idx == len(streams_for_alignment) - 1:
        ...
        combined_array = device_arrays[output_stream_index]
